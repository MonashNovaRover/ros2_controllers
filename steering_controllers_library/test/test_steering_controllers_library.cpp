...
TEST_F(SteeringControllersLibraryTest, test_both_update_methods_for_ref_timeout)
{
  ...
  EXPECT_TRUE(std::isnan((*(controller_->input_ref_twist_.readFromNonRT()))->twist.linear.x));
  EXPECT_TRUE(std::isnan((*(controller_->input_ref_twist_.readFromNonRT()))->twist.angular.z));
